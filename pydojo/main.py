...
# SCREEN
def screen(w, h, opengl=False):
    screen_info.resolution = [w, h]
    if opengl:
        screen_info.screen = pygame.display.set_mode([w, h], OPENGL | DOUBLEBUF | OPENGLBLIT)
        screen_info.opengl = True
    else:
...
def update():
    ...
    if not screen_info.has_background and not screen_info.opengl:
        screen_info.screen.fill(screen_info.bg_color)
    # Draw background if needed
    elif not screen_info.opengl:
